...
- Entry: `lib/main.dart` — Get service registration order matters: `AppLogService → HiveService → DeviceInfoService → Settings/CloudModel → Inference/Cloud/Download/LocalImage → CrashReporting → notifications → Server/ModelController`. Routes: `lib/core/routes.dart`, keys/defaults: `lib/core/constants.dart`. First launch auto-setup runs post-frame: `ModelController.ensureEdgeDefault()` downloads + loads Eburon Edge (SmolLM2-360M Q4_K_M, ~258 MB, no thinking delay) when no local model is selected, and `TtsService` warms the selected voice engine while prefetching the Lite bundle — all into app-private on-device storage, paths re-resolved every launch (never stored). Chat recalls past conversations into the system prompt (`ChatController._memoryBlock`/`buildMemoryBlock`, unit-tested, toggle `memory_enabled` default-on, caps in `AppConstants.memory*`); current chat always dominates context.
