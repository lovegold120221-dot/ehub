...
class SettingsView extends GetView<SettingsController> {
    ...
    return Scaffold(
      ...
      body: Obx(() => ListView(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              _sectionLabel(context, 'VOICE & READ ALOUD'),
              _buildTtsCard(context, isDark),
              const SizedBox(height: 24),
                        ...
                        children: [
                          ...
                          Text(
                              controller.appVersion.value.isEmpty
                                  ? 'Version unavailable · by MasterE'
                                  : 'v${controller.appVersion.value} · by MasterE',
                              style: GoogleFonts.inter(
                                  fontSize: 13,
                                  color: Theme.of(context).hintColor)),
                        ]),
  ...
  Widget _buildTtsCard(BuildContext context, bool isDark) {
    final tts = Get.find<TtsService>();
    return Obx(() {
      final lang = tts.language.value;
      ...
      final langName =
          AppConstants.supertonicLanguages[lang] ?? lang.toUpperCase();
      ...
      return Column(children: [
        _appleGroupedCard(context, isDark, children: [
          _appleListTile(
            context,
            isDark,
            leading: _iconBox(const Color(0xFFAF52DE), Icons.record_voice_over),
            title: 'Voice engine',
            subtitle: tts.engine.value ==
                    AppConstants.ttsEngineLite
                ? 'EburonVoix-Lite · tiny Flemish voice, runs anywhere'
                : 'EburonVoix-3 · best quality · needs a mid-range phone',
            trailing: dropdown<String>(
              value: tts.engine.value,
              items: const [
                DropdownMenuItem(
                    value: AppConstants.ttsEngineSupertonic3,
                    child: Text('EburonVoix-3')),
                DropdownMenuItem(
                    value: AppConstants.ttsEngineLite,
                    child: Text('EburonVoix-Lite')),
              ],
              onChanged: (v) {
                if (v != null) tts.setEngine(v);
              },
            ),
            onTap: () {},
          ),
          _appleListTile(
            context,
            isDark,
            leading: _iconBox(AppColors.primary, Icons.language),
            title: 'Language',
            subtitle:
                '$langName · ${AppConstants.supertonicLanguages.length} supported by EburonVoix-3',
            trailing: dropdown<String>(
              value: AppConstants.supertonicLanguages.containsKey(lang)
                  ? lang
                  : 'en',
              items: AppConstants.supertonicLanguages.entries
                  ...
                  .toList(),
              onChanged: (v) {
                if (v != null) tts.setLanguage(v);
              },
            ),
            showDivider:
                tts.engine.value == AppConstants.ttsEngineSupertonic3,
            onTap: () {},
          ),
          if (tts.engine.value == AppConstants.ttsEngineSupertonic3)
            _appleListTile(
              context,
              isDark,
              leading: _iconBox(
                  const Color(0xFFFF2D55), Icons.graphic_eq_rounded),
              title: 'EburonVoix-3 voice',
              subtitle: 'Neural voice style · F female · M male',
              trailing: dropdown<String>(
                value: AppConstants.supertonicNeuralVoices
                        .contains(tts.neuralVoice.value)
                    ? tts.neuralVoice.value
                    : AppConstants.defaultTtsNeuralVoice,
                items: AppConstants.supertonicNeuralVoices
                    .map((v) =>
                        DropdownMenuItem(value: v, child: Text(v)))
                    .toList(),
                onChanged: (v) {
                  if (v != null) tts.setNeuralVoice(v);
                },
              ),
              showDivider: false,
              onTap: () {},
        ...
        _buildSlider(
          ...
          divisions: 13,
          safeMax: AppConstants.maxTtsRate,
          displayValue: '${tts.rate.value.toStringAsFixed(2)}×',
          icon: Icons.speed,
          warning: '',
        ...
        RadioGroup<bool>(
          ...
          _appleListTile(
            context,
            isDark,
            leading: _iconBox(AppColors.secondary, Icons.play_arrow_rounded),
            title: 'Test voice',
            subtitle: 'Hear the current language and voice',
            trailing: const Icon(Icons.chevron_right, size: 18),
            onTap: () => tts.testVoice(),
          ),
          _appleListTile(
            ...
            isDark,
            leading: _iconBox(
                const Color(0xFFFF9500), Icons.cloud_download_outlined),
            title: 'EburonVoix-3 voice model',
            subtitle: tts.supertonicStatusText,
            trailing: tts.supertonicDownloading.value
                ? SizedBox(
                    width: 20,
                    height: 20,
                ...
                : tts.supertonicPresent.length >= tts.supertonicTotalFiles
                    ? const Icon(Icons.check_circle_outline,
                        size: 20, color: AppColors.success)
                    : const Icon(Icons.download_outlined, size: 20),
            showDivider: false,
            onTap: tts.supertonicDownloading.value
                ? () => tts.cancelSupertonicDownload()
                : tts.supertonicPresent.length >= tts.supertonicTotalFiles
                    ? null
                    : () async {
                        try {
                          await tts.downloadSupertonicAssets();
                          Get.snackbar('EburonVoix-3',
                              'Voice model assets downloaded.',
                              snackPosition: SnackPosition.BOTTOM);
                        } catch (e) {
          ...
          _appleListTile(
            ...
            isDark,
            leading: _iconBox(
                const Color(0xFF34C759), Icons.cloud_download_outlined),
            title: 'EburonVoix-Lite voice',
            subtitle: 'Piper nl_BE-nathalie · ${tts.liteStatusText}',
            trailing: tts.liteState.value == 'downloading'
                ? SizedBox(
                    width: 20,
                    height: 20,
                    child: CircularProgressIndicator(
